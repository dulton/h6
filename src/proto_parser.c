...
void
free_proto_parser(proto_parser *parser)
{
	obj_unref(parser);
}
...
msg_t *
parse_proto(proto_parser *p, uint8_t *data, uint32_t len, int32_t *err)
{
	msg_t *m = NULL;
...
msg_t *
parse_proto_io(proto_parser *p, void *io, int32_t *err)
{
	msg_t *m = NULL;
